...
const prior=JSON.parse(readFileSync(root+"armor-v1-support-audit.json","utf8"));
...
const metadataPaths=fields.filter(f=>f.path.startsWith("metadata.")||f.path.startsWith("knowledge.metadata.")).map(f=>{
 ...
 return {...f,count:affected.length,itemIds:affected.map(i=>i.id),values:affected.map(i=>({itemId:i.id,dungeon:classifyArmorMetadata(i,scope).find(m=>(m.location==="item"?"metadata.":"knowledge.metadata.")+m.key===f.path),general:classifyArmorMetadata(i,{...scope,context:"general"}).find(m=>(m.location==="item"?"metadata.":"knowledge.metadata.")+m.key===f.path)})),recommendation:["knowledge.metadata.displayName","knowledge.metadata.internalName","knowledge.metadata.modVersion","metadata.rarity_salvageable","metadata.salvages","metadata.color"].includes(f.path)?"EXTEND_SAME_PROVIDER_LOCATION_SHAPE_TO_GENERAL_UPGRADE_ONLY":"KEEP_BLOCKING"};
});
