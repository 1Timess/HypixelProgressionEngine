...
const report={snapshot:capture.snapshot,paidModelCalls:0,candidates:capture.evidence.candidates.length,retained:e.candidates.length,
...
writeFileSync("data/armor-integration/set-resumption-audit.json",JSON.stringify(report,null,2)+"\n");
console.log(JSON.stringify({candidates:report.candidates,retained:report.retained,promotedEffectRecords:transitions,bytes,status:report.status,pairs:result.audit.pairLocal,unknownStatPairs:result.audit.comparability.statContract.unknownStatPairCount}));
